...
const SLOTS = [
  ...
  },
  {
    ...
    apiType: 'Smart Home',
    label: 'Smart Home',
    sublabel: 'Licht · Waschmaschine · Spülmaschine · Thermostat · Steckdose',
    icon: '🏠',
    accent: '#14b8a6',
    bg: 'rgba(20,184,166,0.06)',
    border: 'rgba(20,184,166,0.22)',
    match: ['smart home','smarthome','home','licht','waschmaschine'],
    protos: ['mqtt','cloud'] as Proto[],
    defaultPort: 1883,
    mqttTopicDefault: 'smarthome/#',
    fields: {
      mqtt:  ['mqttBroker','mqttPort','mqttTopic','mqttUser','mqttPass','mqttTls'],
      cloud: ['deviceId','apiBaseUrl','apiKey'],
    },
    protoLabel: { mqtt:'MQTT', cloud:'Cloud API' },
    description: 'Steuert Smart-Home-Geräte via MQTT oder Cloud-API. Ein/Aus, Echtzeit-Messwerte, Live-Status.',
    protocols_info: 'MQTT · REST · Matter · Zigbee',
    standard: 'Matter 1.3 · Zigbee 3.0',
  },
] as const;
...
const SH_DEFAULTS: Omit<SHDevice, 'on' | 'power_w' | 'voltage_v' | 'current_a' | 'temp_c' | 'online'>[] = [
  { id:'light_1',      name:'Wohnzimmer Licht',    icon:'💡', category:'Beleuchtung' },
  { id:'light_2',      name:'Schlafzimmer Licht',  icon:'💡', category:'Beleuchtung' },
  { id:'washing',      name:'Waschmaschine',        icon:'🫧', category:'Haushaltsgerät' },
  { id:'dishwasher',   name:'Spülmaschine',         icon:'🍽️', category:'Haushaltsgerät' },
  { id:'thermostat',   name:'Thermostat',            icon:'🌡️', category:'Heizung' },
  { id:'socket_1',     name:'Steckdose Küche',      icon:'🔌', category:'Steckdose' },
  { id:'socket_2',     name:'Steckdose Büro',       icon:'🔌', category:'Steckdose' },
  { id:'heatpump',     name:'Wärmepumpe',           icon:'♨️', category:'Heizung' },
];

const ToggleSwitch: React.FC<{ on: boolean; disabled?: boolean; onChange: () => void; accent: string }> = ({ on, disabled, onChange, accent }) => (
  <button
    type="button"
    onClick={onChange}
    disabled={disabled}
    ...
    style={{
      position:'relative', width:44, height:24, borderRadius:999, border:'none', cursor: disabled ? 'default' : 'pointer',
      background: on ? `linear-gradient(90deg,${accent},${accent}cc)` : 'rgba(255,255,255,0.1)',
      transition:'background .25s ease', flexShrink:0, outline:'none',
...
const SmartHomePanel: React.FC<{ accent: string; deviceId?: string; isDemo?: boolean }> = ({ accent, deviceId, isDemo }) => {
  ...
  // Simulate / poll live data every 4 seconds
  useEffect(() => {
    const pollLive = async () => {
      try {
        // Try real API first; fall back to simulation if unavailable
        const url = deviceId
          ? `${API_URL}/devices/${deviceId}/smarthome/status`
          : null;
        if (url) {
          ...
          if (res.ok) {
            ...
            if (Array.isArray(data.devices)) {
              setDevices(prev => prev.map(d => {
                const live = data.devices.find((ld: SHDevice) => ld.id === d.id);
      ...
      // Simulation: randomize power for "on" devices
      setDevices(prev => prev.map(d => {
        if (!d.on) return { ...d, power_w: 0, online: true };
        const basePower: Record<string, number> = {
          light_1:8, light_2:8, washing:1800, dishwasher:1200,
          thermostat:0, socket_1:60, socket_2:45, heatpump:2400,
        };
        const base = basePower[d.id] ?? 50;
        return {
          ...d,
          power_w:   Math.round(base * (0.92 + Math.random() * 0.16)),
          voltage_v: Math.round((229 + Math.random() * 3) * 10) / 10,
          current_a: base > 0 ? Math.round((base / 230) * (0.9 + Math.random() * 0.2) * 100) / 100 : 0,
          temp_c:    d.category === 'Heizung' ? Math.round((21 + Math.random() * 2) * 10) / 10 : null,
          online:    true,
        };
      }));
      setLastPoll(new Date());
  ...
  const handleToggle = async (id: string) => {
    setToggling(p => { const n = new Set(p); n.add(id); return n; });
    const dev = devices.find(d => d.id === id);
    const newState = dev ? !dev.on : true;
    try {
      if (deviceId) {
        await fetch(`${API_URL}/devices/${deviceId}/smarthome/${id}/control`, {
          method: 'POST',
    ...
    } catch { /* optimistic update regardless */ }
    setDevices(prev => prev.map(d => d.id === id ? { ...d, on: newState, power_w: newState ? d.power_w : 0 } : d));
    setToggling(p => { const n = new Set(p); n.delete(id); return n; });
  };

  const categories = ['Alle', ...Array.from(new Set(SH_DEFAULTS.map(d => d.category)))];
  const visible = filter === 'Alle' ? devices : devices.filter(d => d.category === filter);
  const totalOn = devices.filter(d => d.on).length;
  const totalPower = devices.reduce((s, d) => s + (d.power_w ?? 0), 0);
  ...
  return (
    <div style={{ padding:'0 20px 20px', borderTop:'1px solid rgba(20,184,166,0.12)' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', flexWrap:'wrap', gap:10, padding:'12px 0 14px' }}>
        ...
        <span style={{ marginLeft:'auto', fontSize:10, color:accent, fontFamily:'monospace', fontWeight:700,
          background:`${accent}14`, border:`1px solid ${accent}28`, borderRadius:999, padding:'3px 10px' }}>
          {totalOn} EIN · {totalPower > 0 ? (totalPower / 1000).toFixed(2) + ' kW' : '0 W'} Gesamt
        </span>
        {lastPoll && (
          <span style={{ fontSize:9, color:'rgba(248,250,252,0.22)', fontFamily:'monospace' }}>
            ● Live {lastPoll.toLocaleTimeString('de-DE', { hour:'2-digit', minute:'2-digit', second:'2-digit' })}
          </span>
        )}
      </div>
      ...
      <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:14 }}>
        {categories.map(cat => (
          <button key={cat} type="button" onClick={() => setFilter(cat)} style={{
            fontSize:10, fontWeight:700, borderRadius:999, padding:'4px 12px', cursor:'pointer', outline:'none',
            background: filter === cat ? accent : 'rgba(255,255,255,0.04)',
            color: filter === cat ? '#0a1a18' : 'rgba(248,250,252,0.4)',
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
        {visible.map(dev => {
          const isOn = dev.on;
          const isBusy = toggling.has(dev.id);
          return (
            <div key={dev.id} style={{
              display:'flex', alignItems:'center', gap:10, flexWrap:'wrap',
              background: isOn ? `${accent}0a` : 'rgba(255,255,255,0.025)',
              border: `1px solid ${isOn ? accent + '35' : 'rgba(255,255,255,0.06)'}`,
              borderRadius:12, padding:'10px 14px',
              transition:'all .3s ease',
              boxShadow: isOn ? `0 0 18px ${accent}18` : 'none',
            }}>
              {/* Icon + live dot */}
              <div style={{ position:'relative', fontSize:20, flexShrink:0 }}>
                {dev.icon}
                <div style={{
                  position:'absolute', bottom:-2, right:-2, width:7, height:7, borderRadius:'50%',
                  background: !dev.online ? '#6b7280' : isOn ? '#22c55e' : '#f87171',
                  border:'1px solid rgba(10,16,40,0.8)',
                  animation: isOn && dev.online ? 'wai-breathe 2.5s ease-in-out infinite' : 'none',
                }}/>
              </div>

              {/* Name + category */}
              <div style={{ flex:'1 1 120px', minWidth:0 }}>
                <div style={{ fontSize:13, fontWeight:700, color: isOn ? '#f8fafc' : 'rgba(248,250,252,0.55)', lineHeight:1.2 }}>
                  {dev.name}
                </div>
                <div style={{ fontSize:10, color:'rgba(248,250,252,0.28)', marginTop:1 }}>{dev.category}</div>
              </div>

              {/* Live metrics */}
              <div style={{ display:'flex', gap:8, flexWrap:'wrap', flexShrink:0 }}>
                <div style={{ textAlign:'center', minWidth:46 }}>
                  <div style={{ fontSize:13, fontWeight:800, color: isOn ? accent : 'rgba(248,250,252,0.2)', fontFamily:'monospace' }}>
                    {isOn && dev.power_w !== null ? `${dev.power_w}W` : '—'}
                  </div>
                  <div style={{ fontSize:9, color:'rgba(248,250,252,0.25)', textTransform:'uppercase', letterSpacing:'0.1em' }}>Leistung</div>
                </div>
                {dev.voltage_v !== null && isOn && (
                  <div style={{ textAlign:'center', minWidth:46 }}>
                    <div style={{ fontSize:13, fontWeight:800, color:'rgba(248,250,252,0.55)', fontFamily:'monospace' }}>
                      {dev.voltage_v}V
                    </div>
                    <div style={{ fontSize:9, color:'rgba(248,250,252,0.25)', textTransform:'uppercase', letterSpacing:'0.1em' }}>Spannung</div>
                  </div>
                )}
                {dev.current_a !== null && isOn && (
                  <div style={{ textAlign:'center', minWidth:46 }}>
                    <div style={{ fontSize:13, fontWeight:800, color:'rgba(248,250,252,0.55)', fontFamily:'monospace' }}>
                      {dev.current_a}A
                    </div>
                    <div style={{ fontSize:9, color:'rgba(248,250,252,0.25)', textTransform:'uppercase', letterSpacing:'0.1em' }}>Strom</div>
                  </div>
                )}
                {dev.temp_c !== null && isOn && (
                  <div style={{ textAlign:'center', minWidth:46 }}>
                    <div style={{ fontSize:13, fontWeight:800, color:'#f59e0b', fontFamily:'monospace' }}>
                      {dev.temp_c}°C
                    </div>
                    <div style={{ fontSize:9, color:'rgba(248,250,252,0.25)', textTransform:'uppercase', letterSpacing:'0.1em' }}>Temp.</div>
                  </div>
                )}
              </div>
              ...
              <div style={{
                ...
                background: isOn ? `${accent}20` : 'rgba(248,82,82,0.12)',
                color: isOn ? accent : '#f87171',
                border: `1px solid ${isOn ? accent + '40' : 'rgba(248,82,82,0.25)'}`,
              ...
              {/* Toggle */}
              <ToggleSwitch on={isOn} disabled={isBusy} onChange={() => handleToggle(dev.id)} accent={accent} />
            </div>
          );
        })}
      </div>

      {/* Alles aus / alles ein buttons */}
      <div style={{ display:'flex', gap:8, marginTop:14 }}>
        <button type="button" onClick={() => devices.forEach(d => !d.on && handleToggle(d.id))}
          style={{ flex:1, padding:'9px 0', borderRadius:999, border:`1px solid ${accent}35`, background:`${accent}12`,
            color:accent, fontWeight:700, fontSize:11, cursor:'pointer', outline:'none', transition:'all .2s ease' }}>
          ✓ Alle einschalten
        </button>
        <button type="button" onClick={() => devices.forEach(d => d.on && handleToggle(d.id))}
          style={{ flex:1, padding:'9px 0', borderRadius:999, border:'1px solid rgba(248,82,82,0.3)', background:'rgba(248,82,82,0.07)',
            color:'#f87171', fontWeight:700, fontSize:11, cursor:'pointer', outline:'none', transition:'all .2s ease' }}>
          ✕ Alle ausschalten
        </button>
      </div>
    </div>
