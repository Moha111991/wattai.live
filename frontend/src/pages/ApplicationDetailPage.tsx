...
import { getLocalizedApplicationBySlug } from '../data/applications';
import { useLanguage } from '../context/LanguageContext';

export default function ApplicationDetailPage() {
  const { language } = useLanguage();
  const { slug } = useParams<'slug'>();
  const application = slug ? getLocalizedApplicationBySlug(slug, language) : undefined;

  if (!application) {
    return (
      <main style={{ minHeight: '100dvh', background: '#020617', color: '#e2e8f0', padding: '48px 20px' }}>
        <div style={{ maxWidth: 820, margin: '0 auto' }}>
          <h1 style={{ margin: 0, fontSize: 'clamp(26px,4vw,42px)' }}>
            {language === 'en' ? 'Application not found' : 'Anwendung nicht gefunden'}
          </h1>
          <p style={{ color: '#94a3b8', lineHeight: 1.7 }}>
            {language === 'en'
              ? 'This application does not exist or has been moved.'
      ...
      <section style={{ maxWidth: 980, margin: '0 auto', display: 'grid', gap: 20 }}>
        ...
        <article style={{
          ...
          <h2 style={{ margin: '0 0 14px', fontSize: 'clamp(22px,3vw,32px)', color: '#f8fafc' }}>
            {language === 'en' ? 'Deep technical description' : 'Tiefe technische Beschreibung'}
          </h2>
